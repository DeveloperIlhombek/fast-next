...
export default function AttendancePage() {
	...
	useEffect(() => {
		Promise.all([teacherApi.getGroupStudents(id), teacherApi.getMyGroups()])
			.then(([studs, groups]) => {
				setStudents(studs)
	...
	const handleMarkAll = async (status: 'present' | 'absent') => {
		...
		const confirmed = await tgConfirm(`${label} deb belgilash?`)
		if (confirmed) {
			haptic.medium()
	...
	const filteredStudents =
		filter === 'unmarked' ? students.filter(s => !getStatus(s.id)) : students
				...
				<div style={{ display: 'flex', gap: '8px' }}>
					<button
						style={{
							...
							background:
								'color-mix(in srgb, var(--status-present) 10%, transparent)',
				...
				{[
					...
					<button
						...
						style={{
							...
							cursor: 'pointer',
							backgroundColor:
								filter === key ? 'var(--tg-button)' : 'var(--tg-secondary-bg)',
							color:
